...
//  Get worksheet dimensions
        ...
        $controlRowData = $sheet->rangeToArray('A' . 8 . ':' . $highestColumn . 8, NULL, TRUE, FALSE);
        //figuring out real highest column
        $colNumber = PHPExcel_Cell::columnIndexFromString($highestColumn);
        $colNumber--;
        while (is_null($controlRowData[0][$colNumber])) {
            $colNumber--;
        }
        $highestColumn = PHPExcel_Cell::stringFromColumnIndex($colNumber);
        $controlRowData = $sheet->rangeToArray('A' . 8 . ':' . $highestColumn . 8, NULL, TRUE, FALSE);
...
//  Loop through each row of the worksheet in turn
        for ($row = 9; $row <= $highestRow; $row++) {
            ...
            if (is_null($rowData[0][0])) {//figure real end of serial number rows
                break;
            }
            $tmp_saver = [];
            $tmp_saver[0] = $rowData[0][1];
            $tmp_saver[1] = [];
            $tmp_saver[1][0] = $controlRowData[0];
            $tmp_saver[1][1] = $rowData[0];
            array_push($to_save_data, $tmp_saver);
        }
        $tmpData = $sheet->rangeToArray('A' . 5 . ':' . $highestColumn . 5, NULL, TRUE, FALSE);
        $_SESSION['marklist_courseid'] = $tmpData[0][1];
        $_SESSION['marklist_data_to_process'] = $to_save_data;
